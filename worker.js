  ...
  async fetch(request, env) {
    ...
    if (request.method === "GET") {
      return jsonResponse(
        {
          success: true,
          message: "JuaStore Garansi API aktif."
        },
        200,
        corsHeaders
      );
    ...
    try {
      if (!env.TELEGRAM_BOT_TOKEN || !env.TELEGRAM_CHAT_ID) {
        throw new Error(
          "TELEGRAM_BOT_TOKEN atau TELEGRAM_CHAT_ID belum dipasang."
        );
      }
        ...
        return jsonResponse(
          {
            success: false,
            message:
              "Nama, nomor WhatsApp, produk, dan kendala wajib diisi."
          },
          400,
          corsHeaders
        );
...
async function telegramJsonRequest(botToken, method, payload) {
  ...
  if (!response.ok || !result.ok) {
    throw new Error(
      result.description || `Telegram ${method} gagal.`
    );
  }
...
async function sendPhotoFromDataUrl(
  botToken,
  chatId,
  dataUrl,
  caption
  ...
  if (!match) {
    throw new Error(
      "Format screenshot harus JPG, PNG, atau WEBP."
    );
  }
  ...
  if (bytes.byteLength > 5 * 1024 * 1024) {
    throw new Error(
      "Ukuran screenshot maksimal 5 MB."
    );
  }
  ...
  formData.append(
    ...
    `bukti-garansi.${extensionMap[mimeType]}`
  );
  ...
  if (!response.ok || !result.ok) {
    throw new Error(
      result.description || "Screenshot gagal dikirim."
    );
  }
}
...
function createWarrantyId() {
  ...
  return `GRN-${stamp}-${String(randomValue).padStart(4, "0")}`;
}
